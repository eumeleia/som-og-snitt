...
export function kroppsDel(k: TKonstruksjon): Del {
  ...
  return {
    ...
    punkter: Object.entries(P).map(([navn, punkt]) => ({ navn, punkt })),
  }
}
...
export function ermDel(k: TKonstruksjon, langt = true): Del {
  ...
  return {
    ...
    // erm[0]/erm[1] ligger på senterlinja, ikke på klippelinja. P3 er
    // ermkulens toppunkt (punkt 3), kule[12] er pitch point (punkt 6,
    // t=12/36=2/6) — samme punkt som hakket over, navngitt for verifisering.
    punkter: [
      ...Object.entries(erm).map(([navn, punkt]) => ({ navn, punkt })),
      { navn: '3', punkt: P3 },
      { navn: '6', punkt: kule[12] },
    ],
  }
}
