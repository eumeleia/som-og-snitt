 ...
 *
 * Forstykket har egen skulderlinje (18) og eget ærmegab (gjennom 17),
...
const P67: [number, number][] = [[1.5, 2.5], [1.75, 3], [2, 3.5]]        // ærmegabdybde +
...
export function konstruer(m: KroppMaal, type: Blokktype = 'kropp'): KroppKonstruksjon {
  ...
  P[7] = { x: 0, y: P[6].y + m.aermegabDybde + P67[g][i] }        // ærmegabdybdelinje
  P[8] = { x: P[3].x, y: P[7].y }
  P[9] = { x: 0, y: P[6].y + (P[7].y - P[6].y) / 2 }              // ½ av 6–7
  P[10] = { x: 0, y: P[6].y + m.aermegabDybde / 4 - 2 }           // ¼ ærmegabdybde − 2
...
/** Ærmegab. Bak 15 → 13 → 8. Foran 18 → 17 → 8. */
function aermegab(k: KroppKonstruksjon, side: Side): Punkt[] {
  const { P } = k
 ...
 * punktene i `P` (også de som ikke ligger på selve klippelinja), så avstanden
 * mellom to hvilke som helst punkter kan måles på det projiserte bildet og
 * sammenlignes direkte med boka. Samme tilnærming som babyblokk.ts.
 */
function konstruksjonspunkter(k: KroppKonstruksjon): { navn: string; punkt: Punkt }[] {
...
export function del(k: KroppKonstruksjon, side: Side, tilHofte = true): Del {
  ...
  return {
    ...
    // Boka merker et pitch point i ærmegapet (13 bak, 17 foran) i tillegg til
    // midjehakket — det er der ermets eget pitch-hakk (se ermDel) skal møte
    // kroppen. Uten dette hakket her er det ingen måte å se på selve arket
    // hvor ermet skal festes.
    hakk: [{ punkt: P[1], retning: 1 }, { punkt: side === 'bak' ? P[13] : P[17], retning: 1 }],
    punkter: konstruksjonspunkter(k),
  }
}

export const aermegabLengde = (k: KroppKonstruksjon, side: Side) => lengde(aermegab(k, side))

/** Samlet ærmegab, bak pluss front. Ermets punkt 3 avhenger av det. */
export const aermegabTotal = (k: KroppKonstruksjon) =>
  aermegabLengde(k, 'bak') + aermegabLengde(k, 'front')
...
export function ermDel(k: KroppKonstruksjon): Del {
  ...
  const A = aermegabLengde(k, 'bak')            // halve ermkulen matcher bakre ærmegab
  if (A <= y1) throw new Error('Ærmegabet er kortere enn ermkulehøyden')
  ...
  return {
    ...
    // Ermets egne konstruksjonspunkter: 0 og 1 ligger på senterlinja (ikke på
    // klippelinja), 3 er ermkulens toppunkt, 4 er håndleddshjørnet, 6 er
    // pitch point — samme punkt som hakket over, men navngitt for
    // verifisering mot boka.
    punkter: [
      { navn: '0', punkt: { x: 0, y: 0 } },
      { navn: '1', punkt: { x: 0, y: y1 } },
      { navn: '2', punkt: { x: 0, y: y2 } },
      { navn: '3', punkt: P3 },
      { navn: '4', punkt: { x: x4, y: y2 } },
      { navn: '6', punkt: pitch },
    ],
  }
}
...
export function valider(k: KroppKonstruksjon): string[] {
  ...
  if (P[2].y <= P[7].y) feil.push('Hoftelinjen ligger over ærmegabet')
  const b = aermegabLengde(k, 'bak'), f = aermegabLengde(k, 'front')
  if (Math.abs(b - f) > b * 0.15)
    feil.push(`Ærmegabene avviker mye: bak ${b.toFixed(1)}, foran ${f.toFixed(1)}`)
  return feil
}
