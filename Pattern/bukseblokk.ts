...
export function tilDel(k: Konstruksjon): Del {
  ...
  return {
    ...
    // Forstykket (0–11) er speilet inn i selve konturen (se `front` over) — de
    // samme punktene må speiles her også, ellers havner de på feil side av
    // brettelinjen i forhold til den tegnede linjen. Bakstykket (12–18)
    // brukes uspeilet i konturen og skal derfor stå uspeilet her også.
    punkter: [
      ...Object.entries(P).filter(([n]) => +n <= 11).map(([navn, punkt]) => ({ navn, punkt: S(punkt) })),
      ...Object.entries(P).filter(([n]) => +n >= 12).map(([navn, punkt]) => ({ navn, punkt })),
    ],
  }
}
